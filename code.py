...
# Page tracking
pages = ["Joules Counter", "CMD Counters", "Infect Counter", "Speed Tracker", "Battery Voltage", "Sleep Timer"]
...
# Sleep timeout (display refresh only)
inactivity_timeout = 15

# Flash state for low battery indicator
flash_state = False
...
def update_display():
    ...
    global joules_counter, cmd1_counter, cmd2_counter, cmd3_counter, infect_counter, speed_counter, inactivity_timeout
    ...
    if selected_page is None:
        ...
        if selected_page == "Joules Counter":
            ...
            counter_text = centered_label(font, str(infect_counter), 25, scale=2 if infect_counter <= 9 else 1)
            display_group.append(counter_text)
            if infect_counter == 10:
                display_group.append(centered_label(font, "COMPLEATED", 50))
                counter_text.x = 55
        elif selected_page == "Speed Tracker":
            label_text = "Speed" if speed_counter <= 3 else "MAX SPEED"
            display_group.append(centered_label(font, label_text, 5))
        ...
        elif selected_page == "Sleep Timer":
            display_group.append(centered_label(font, "Sleep Delay", 5))
            display_group.append(centered_label(font, f"{inactivity_timeout}s", 30, scale=2))
...
def handle_button_press(name):
    ...
    if selected_page == "Joules Counter":
        if name == "B1": joules_counter += 1
        elif name == "B2": joules_counter -= 1
        elif name == "B3": joules_counter += 2
        elif name == "B4": joules_counter -= 2
        elif name == "B5": joules_counter += 3
        elif name == "B6": joules_counter = 0
    elif selected_page == "CMD Counters":
        if name == "B1": cmd1_counter += 1
        elif name == "B2": cmd1_counter -= 1
        elif name == "B3": cmd2_counter += 1
        elif name == "B4": cmd2_counter -= 1
        elif name == "B5": cmd3_counter += 1
        elif name == "B6": cmd3_counter -= 1
        elif name == "B7": cmd1_counter = cmd2_counter = cmd3_counter = 0
    elif selected_page == "Infect Counter":
        if name == "B1": infect_counter = min(infect_counter + 1, 10)
        elif name == "B2": infect_counter = max(infect_counter - 1, 0)
        elif name == "B6": infect_counter = 0
    elif selected_page == "Speed Tracker":
        if name == "B1": speed_counter = min(speed_counter + 1, 4)
        elif name == "B2": speed_counter = max(speed_counter - 1, 1)
        elif name == "B7": speed_counter = 0
    elif selected_page == "Sleep Timer":
        if name == "B1": inactivity_timeout = min(inactivity_timeout + 5, 300)
        elif name == "B2": inactivity_timeout = max(inactivity_timeout - 5, 5)
